      ...
      <div className="container mx-auto px-4 max-w-6xl">
        ...
        {!hasTours ? (
          ...
          <div className="space-y-16">
            ...
            {/* Individual Shows — grouped by artist */}
            {tourDates.length > 0 && (() => {
              // Group tour dates by artist
              const grouped = new Map<number, { artist: TourDateWithArtist["artist"]; shows: TourDateWithArtist[] }>();
              for (const td of tourDates) {
                if (!grouped.has(td.artist.id)) {
                  grouped.set(td.artist.id, { artist: td.artist, shows: [] });
                }
                grouped.get(td.artist.id)!.shows.push(td);
              }
              ...
              return (
                <section>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    {Array.from(grouped.values()).map(({ artist, shows }) => {
                          ...
                          <div className="card card-hover overflow-hidden">
                            ...
                            <div className="p-5">
                              ...
                              <h3 className="font-semibold text-slate-900 group-hover:text-primary-600 transition-colors mb-3">
                                {shows[0].description || "On Tour"}
                              </h3>
                              <div className="space-y-1.5">
                                {shows.map((show) => (
                                  <div key={show.id} className="flex items-center gap-3 text-slate-500 text-sm">
