...
export default function CommunityRegistrationPage() {
  ...
  const checkDuplicateName = useCallback(async (name: string) => {
    ...
    try {
      const response = await fetch(
        `${API_URL}/hosts/check-name?name=${encodeURIComponent(name)}`
      );
      if (response.ok) {
        const data = await response.json();
        if (data.exists) {
  ...
  const validateForm = () => {
    ...
    if (!formData.communityName || formData.communityName.length < 2) {
      newErrors.communityName = "Host name is required";
      ...
      const response = await fetch(
        `${API_URL}/hosts`,
        {
          ...
          body: JSON.stringify({
            ...
            receive_artist_offers: formData.receiveArtistOffers,
          }),
        }
      );
            ...
            <div className="space-y-6">
              ...
              <div>
                <label className="flex items-start gap-3 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={formData.receiveArtistOffers}
                    onChange={(e) =>
                      setFormData({ ...formData, receiveArtistOffers: e.target.checked })
                    }
                    className="mt-1 w-5 h-5 text-teal-500 border-2 border-slate-300 rounded focus:ring-teal-400"
                  />
                  <span className="text-slate-700 text-base">
                    Would you like to receive direct offers from artists who are visiting your area?
                  </span>
                </label>
              </div>
            </div>
