...
export default function ArtistRegistrationPage() {
  ...
  const validateForm = () => {
    ...
    if (!formData.artistName || formData.artistName.length < 2) {
      newErrors.artistName = "Talent/Stage name is required";
    ...
    for (const { key, label } of urlFields) {
      ...
      if (val && !urlPattern.test(val)) {
        newErrors[key] = `${label} must be a valid URL (starting with http:// or https://)`;
      }
    }
    // Validate video URLs
    const invalidVideos = formData.videoUrls
      .map((url, i) => ({ url, i }))
      .filter(({ url }) => url.trim() && !urlPattern.test(url));
    if (invalidVideos.length > 0) {
      newErrors.videoUrls = `Invalid video URL${invalidVideos.length > 1 ? "s" : ""} at position ${invalidVideos.map(v => v.i + 1).join(", ")}`;
    }
    // Validate Spotify links
    const invalidSpotify = formData.spotifyLinks
      .map((url, i) => ({ url, i }))
      .filter(({ url }) => url.trim() && !urlPattern.test(url));
    if (invalidSpotify.length > 0) {
      newErrors.spotifyLinks = `Invalid Spotify URL${invalidSpotify.length > 1 ? "s" : ""} at position ${invalidSpotify.map(v => v.i + 1).join(", ")}`;
    }
      ...
      <div className="min-h-screen bg-white flex items-center justify-center p-4">
        <div className="max-w-md w-full text-center">
          ...
          <h1 className="text-3xl font-serif font-bold text-slate-900 mb-2">
            {isAgent ? "Talent Submitted!" : "Welcome to Kolamba!"}
          </h1>
          <p className="text-slate-600 mb-2">
            {isAgent
              ? `Talent profile #${submittedArtistsCount} has been created.`
          ...
          {submittedArtistsCount > 0 && isAgent && (
            <div className="mb-6 p-3 bg-teal-50 rounded-lg border border-teal-200">
              <p className="text-teal-700 text-sm font-medium">
                You&apos;ve submitted {submittedArtistsCount} talent{submittedArtistsCount > 1 ? "s" : ""} so far. Continue adding more below.
              </p>
            </div>
          )}
                ...
                <div className="space-y-3">
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (errors.website) setErrors((prev) => { const next = { ...prev }; delete next.website; return next; });
                    }}
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (errors.youtube) setErrors((prev) => { const next = { ...prev }; delete next.youtube; return next; });
                    }}
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (errors.facebook) setErrors((prev) => { const next = { ...prev }; delete next.facebook; return next; });
                    }}
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (errors.linkedin) setErrors((prev) => { const next = { ...prev }; delete next.linkedin; return next; });
                    }}
                    placeholder="LinkedIn profile URL"
                    className="w-full px-4 py-3.5 border-2 border-slate-300 rounded-lg text-base focus:outline-none focus:border-teal-400 transition-colors"
                  />
                </div>
                {(errors.website || errors.youtube || errors.facebook || errors.linkedin) && (
                  <div className="mt-2 space-y-1">
                    {errors.website && <p className="text-sm text-red-500">{errors.website}</p>}
                    {errors.youtube && <p className="text-sm text-red-500">{errors.youtube}</p>}
          ...
          <div className="mt-12 space-y-8">
            ...
            <div>
              ...
              <div className="space-y-2">
                  ...
                  <div key={index} className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        setFormData({ ...formData, videoUrls: newUrls });
                        if (errors.videoUrls) {
                          setErrors((prev) => { const next = { ...prev }; delete next.videoUrls; return next; });
                        }
                      }}
                ...
                {errors.videoUrls && (
                  <p className="mt-2 text-sm text-red-500">{errors.videoUrls}</p>
                )}
              </div>
            </div>
